...
export const createstockManagement =(asyncHandler(async(req,res,next)=>{
    const {name,categoryId,subCategoryId,supplierId}=req.body
    ...
    const stockManagement =await stockManagementModel.create({name,slug:slugify(name),supplierId,image:{secure_url,public_id},createdBy:req.user._id,updatedBy:req.user._id,categoryId,subCategoryId})
